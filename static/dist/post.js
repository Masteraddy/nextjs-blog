      ...
      <div className="row">
        ...
        <div className="maincate">
          {props.post.category.map(data => (
            <Link key={data} href={`/category?id=${data}`}>
              <a>
        ...
        <div className="row">
          {props.npost ? (
            <Link href={`/post?id=${props.npost.url}`}>
              <a
                className="right"
                style={{
                  textDecoration: "none",
          ...
          {props.ppost ? (
            <Link href={`/post?id=${props.ppost.url}`}>
        ...
        <ReactDisqusComment
          ...
          url={`https://jaycruzworld.now.sh/post?id=${props.post.url}`}
...
PostPage.getInitialProps = async ({ query: { id } }) => {
  ...
  const post = await api.showPost(posts, id);
  const index = await api.prevNextPost(posts, id);
  console.log(post);
